...
public class EditProfile extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String oldUserName = request.getParameter("oldUserName").trim();
		User loggedInUser = (User) session.getAttribute("loggedInUser");
		System.out.println((loggedInUser == null || (!loggedInUser.isAdmin() && !loggedInUser.getUserName().equals(oldUserName)) ));
		if(loggedInUser == null || (!loggedInUser.isAdmin() && !loggedInUser.getUserName().equals(oldUserName)) ) {
		...
		UsersDAO.instance.update(u, oldUserName);
		try {
			Thread.sleep(10000);
